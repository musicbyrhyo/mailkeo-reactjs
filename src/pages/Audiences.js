...
export const Audiences = () => {
  ...
  const AudienceList = Audiences.map((audience) => (
    <ChannelB
      title={audience.a_name}
      subscribers={audience.a_subscribers.length}
      id={audience._id}
      date={audience.a_created.slice(0, 10)}
    />
  ));
  ...
  return (
    <>
      ...
      <CampaignHolder>
        <HeaderHolder>
          <Header>Your Audiences</Header>
          <CreateCampaign onClick={ViewCreateAudience}>
            Create New Audience
          </CreateCampaign>
        </HeaderHolder>
        <div>
          <SubHeader>All Audiences</SubHeader>
          <Divider></Divider>
          <div>{AudienceList}</div>
        </div>
      </CampaignHolder>
      <CreateAudience
        Trigger={CreateAudienceTrigger}
        setTrigger={setCreateAudienceTrigger}
      />
    </>
  );
};
